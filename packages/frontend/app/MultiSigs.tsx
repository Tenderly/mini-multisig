...
function MultiSig({ multiSig, multiSigAbi }: MultiSigParams) {
  ...
  const { data, isSuccess, isError } = useBalance({
  ...
  return (
    <div className="space-y-2">
      ...
      <code>
        <small>{multiSig.address}</small>
      </code>
      <div>
        <Button
          onClick={() => setFunding(true)}
          variant="outline"
          data-testid="ms-fund-btn"
        >
          Fund
        </Button>
      </div>
...
function FundMultiSig({
  ...
  return (
    ...
    >
      <DialogContent>
        <DialogHeader>
          <h2>Funding multisig</h2> <small>{multiSig.address}</small>
        </DialogHeader>
        <Input
          type="number"
          value={amount}
        ...
        <DialogFooter>
          <Button
            ...
            data-testid="ms-fund-confirm-btn"
          >
            Fund
          </Button>
    ...
    <div>
      ...
      >
        ...
        <DialogContent>
          ...
          <DialogFooter>
            {!review && (
              <Button onClick={() => setReview(true)} type="button">
                Review
              </Button>
            )}
            {review && (
              <Button
                onClick={() => setTriggerSend(true)}
                disabled={!sendable}
                type="submit"
              >
                Send
            ...
            <Button
              onClick={() => (review ? setReview(false) : reset())}
              type="reset"
...
function Transaction({
  ...
  return (
    <Accordion type="single" collapsible>
      <AccordionItem
        value="item-1"
        data-testid={`ms-transaction-${transaction.txIndex}`}
      >
        <AccordionTrigger>
          {transaction.txIndex}. {transaction.name}{" "}
          {transaction.approvedBy.length} /{" "}
          {multiSigParams.multiSig.signaturesRequired}
        </AccordionTrigger>
        <AccordionContent>
          <div className="mb-3">
      ...
      <DialogContent>
        ...
        <DialogFooter>
          <Button
            ...
            data-testid="ms-tx-execute-confirm"
          >
            Execute
          </Button>
...
function TransactionApproval({
  ...
  return (
    <Dialog open={true} onOpenChange={(open) => !open && onCancelApproval()}>
      <DialogContent>
        ...
        <DialogFooter>
          <Button
            disabled={!write && !prepareContractWriteError}
            onClick={() => {
    ...
    >
      <DialogTrigger asChild>
        <Button
          ...
          data-testid={"ms-create-multisig"}
        >
          Create MutiSig
        </Button>
      </DialogTrigger>
        ...
        <DialogFooter>
          ...
          {reviewing && (
            <Button
              ...
              disabled={!canCreate}
              data-testid="ms-multisig-create"
            >
              Create
            </Button>
          )}
